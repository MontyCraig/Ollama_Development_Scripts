...
async def main() -> None:
    ...
    try:
        ...
        # Validate and sanitize models path
        models_path = Path("/Volumes/One Touch/Ollama_Presentation/ollama_models_list.txt")
        if not models_path.exists():
            raise FileNotFoundError(f"Models file not found at {models_path}")
        
        # Update models list with validation
        models = update_ollama_models_list(str(models_path))
        logger.info(f"Loaded {len(models)} models")
        
        if not models:
            logger.error("No models available - cannot proceed")
            return
