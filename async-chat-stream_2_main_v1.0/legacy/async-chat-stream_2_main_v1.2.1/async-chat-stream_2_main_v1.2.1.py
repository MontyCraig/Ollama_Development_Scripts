...
# Set up logging with path relative to script
log_path = script_dir / 'logs' / 'task_processing.log'
...
# Set default values with paths relative to script directory
default_config = {
    'pre_prompt_path': str(script_dir / 'config' / 'pre_prompt.txt'),
    'post_prompt_path': str(script_dir / 'config' / 'post_prompt.txt'),
    'default_model': "llama2",
    'input_csv_path': str(script_dir / 'input' / 'input.csv'),
    'output_csv_path': str(script_dir / 'output' / 'output.csv'),
    'output_base_path': str(script_dir / 'output')
}
...
# Validate file paths
def validate_file_path(path: Union[str, Path], is_output: bool = False) -> None:
    path = Path(path)
    if not path.is_file():
        if is_output:
...
def create_output_folders(base_path: Union[str, Path], model_name: str, output_type: str) -> Dict[str, Path]:
    base_path = Path(base_path)
    folders = {
        'json_output': base_path / 'chatbot_outputs' / 'model_tests' / model_name / output_type / 'json_output',
        'txt_output': base_path / 'chatbot_outputs' / 'model_tests' / model_name / output_type / 'txt_output',
    }
    for folder in folders.values():
        folder.mkdir(parents=True, exist_ok=True)
    return folders

def create_language_folders(base_path: Union[str, Path], model_name: str, output_type: str) -> tuple[List[str], Dict[str, Path]]:
    base_path = Path(base_path)
    languages = ['python', 'javascript', 'html', 'css', 'sql', 'php']  # Add more languages as needed
    folders = {}
    for lang in languages:
        folder_path = base_path / 'chatbot_outputs' / 'model_tests' / model_name / output_type / f'{lang}_output'
...
def get_unique_filename(file_path: Union[str, Path]) -> Path:
    ...
    while file_path.exists():
        file_path = file_path.parent / f"{base}_{counter}{extension}"
        counter += 1
    return file_path
...
async def process_task(task, output_folders, languages, model):
    ...
    script_dir = Path(__file__).parent.absolute()
    file_paths = {
        'json': get_unique_filename(script_dir / output_folders['json_output'] / f"{file_name}.json"),
        'txt': get_unique_filename(script_dir / output_folders['txt_output'] / f"{file_name}.txt"),
    }

    for lang in languages:
        lang_lower = lang.lower()
        if f'{lang_lower}_output' in output_folders:
            file_paths[lang_lower] = get_unique_filename(script_dir / output_folders[f'{lang_lower}_output'] / f"{file_name}.{lang_lower}")
    ...
    for lang in languages:
        ...
        if f'{lang.lower}_output' in output_folders:
            existing_files[lang] = os.listdir(script_dir / output_folders[f'{lang.lower}_output'])
        ...
        if f'{lang}_output' in output_folders:
            for i, snippet in enumerate(snippets, 1):
                ...
                if file_name not in existing_files[lang]:
                    file_path = script_dir / output_folders[f'{lang}_output'] / file_name
...
async def main():
    ...
    base_path = script_dir / config['output_base_path']
    ...
    last_completed_task = {}
    last_completed_task_path = script_dir / 'last_completed_task.json'
    try:
        with open(last_completed_task_path, 'r') as f:
    ...
    while True:
        ...
        while start_index < total_tasks:
            ...
            for i in range(start_index, min(start_index + 10, total_tasks)):
                ...
                print(f"{i + 1}. Task {task_number} - {status}")
            ...
            if task_choice.lower() == 'n':
                ...
                for i in range(start_index, total_tasks):
                    ...
                    if task not in completed_tasks[selected_group]:
                        ...
                        print(f"\nProcessing task {i + 1}/{total_tasks}: {task_number} - {task_description}")
                        try:
                            ...
                            if metadata:
                                ...
                                with open(last_completed_task_path, 'w') as f:
